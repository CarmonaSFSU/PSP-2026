...
So factors in R are objects of class "factor" composed of an integer vector and  a `$class` attribute and a `$levels` attribute containing a character vector.
...
It can be constructed from a Vector, as supplied by the "data" argument for the  `matrix()` function.
...
You can store a two dimensional collection of data (a "table") as a data frame. This is the most common way to work with data in R. A data frame is often constructed from one or more vectors, where each vector becomes a column.
...
3) Calculate the mean, minimum, and maximum of each vector. 
